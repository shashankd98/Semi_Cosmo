...
def Menc_Ein(r):
    s=(2./alp)**(1./alp)*r/rs
    return 2.*np.pi*ds*rs*rs*rs*np.exp(2./alp)*(alp/2.)**(-1.+3./alp)*gamma(3./alp)*(1.-gammainc(3./alp,s**alp)/gamma(3./alp))
def Phi_NFW(r):
    x=r/R200c; c=R200c/rs
...
def Phi_Ein(r):
    h=rs/(2./alp)**(1./alp)
    s=(2./alp)**(1./alp)*r/rs
    M=2.*np.pi*ds*rs*rs*rs*np.exp(2./alp)*(alp/2.)**(-1.+3./alp)*gamma(3./alp)
    return -G*M/h/s*(1.-gammainc(3./alp,s**alp)/gamma(3./alp)+s*gammainc(2./alp,s**alp)/gamma(3./alp))
...
def main():
    ...
    r=kpc*np.logspace(1,5,500); 
    #sol = odeint( dMbydr, Menc_Ein(1.e-2*R200m), r )
    #sol = odeint( dMbydr, Menc_NFW(1.e-2*R200m), r )
    sol_phi = odeint( dPhibydr, Phi_Ein(1.e-2*R200m), r)
    ...
    f1 = Phi_Ein(r); f2 = Phi_outer(r)-2./3.*np.pi*G*dm*r*r
    ...
    ax1.loglog(r/R200m,phi_fit-phi_fit[0],label="Fit")
    ax1.loglog(r/R200m,phi_true-phi_true[0],label='True')
    ax1.loglog(r/R200m,phi_sol-phi_sol[0],label="Sol")
    ...
    ax2.loglog(rm1/R200m,-g_fit,label="Fit")
    ax2.loglog(rm1/R200m,-g_true,label="True")
    ax2.loglog(rm1/R200m,-g_sol,label="Sol")
    ...
    ax3.loglog(rm2/R200m,rho_fit,label="Fit")
    ax3.loglog(rm2/R200m,rho_true,label="True")
    ax3.loglog(rm2/R200m,rho_sol,label="Sol")
    ax3.set_xlabel("R/$R_{200m}$")
