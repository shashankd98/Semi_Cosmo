...
{
  int i,j,k;
    double mdot_acc, *dx1, *x1, *dx2, *x2, *dx3, *x3;
    mdot_acc=0;
    dx1 = grid->dx[IDIR];
    x1=grid->x[IDIR];
    dx2 = grid->dx[JDIR];
    ...
    JDOM_LOOP(j){
   mdot_acc += x1[IBEG]*x1[IBEG]*sin(x2[j])*dx2[j]*2*CONST_PI*d->Vc[RHO][0][j][IBEG]*d->Vc[VX1][0][j][IBEG];
   }
   if (prank == 0){
    char fname[512];
    static double tpos = -1.0;
    FILE *fp;
    sprintf (fname, "%s/mdot_acc.dat",RuntimeGet()->output_dir);
