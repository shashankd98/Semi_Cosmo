...
void Src_jet (const Data *d, Grid *grid)
{
  ...
  double psi_jet, mdot_acc, darea, norm_jet, Srho, dr_cube, v_jet, Se;
...
/* ************** Mass accretion through the inner radius*************** */
    ...
    DOM_LOOP(k,j,i)
    {
      ...
      // if(d->Vc[VX1][0][j][IBEG]<0. && T<1.e5) mdot_acc += x1[IBEG]*x1[IBEG]*sin(x2[j])*dx2[j]*2*CONST_PI*d->Vc[RHO][0][j][IBEG]*d->Vc[VX1][0][j][IBEG];
      if(T<1.e5 && x1[i]*UNIT_LENGTH<5.e3*CONST_pc) mdot_acc+=x1[i]*x1[i]*sin(x2[j])*dx1[i]*dx2[j]*dx3[k]*d->Vc[RHO][k][j][i];	
    }
    #ifdef PARALLEL
     }
   ...
   mdot_acc=-mdot_acc/tau_dep; 
    ...
    v_jet = sqrt(2.*EFF*f_kin/eta_MLF)*CONST_c/UNIT_VELOCITY; //code units
    TEdot_jet = 0.5*(1.-f_kin)*EFF*mdot_acc*(CONST_c/UNIT_VELOCITY)*(CONST_c/UNIT_VELOCITY); //code units, single-jet Edot_th
