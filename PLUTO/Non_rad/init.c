...
{
    double z=solver(g_time*UNIT_LENGTH/UNIT_VELOCITY);
    double se=1.5-z/6.*0.2;
    double be=1.2+z/6.*3.5;
    double dcr0=3.*H0*H0/(8.*pi*G);
    double dcr=dcr0*Ezsqr(z);
    double M200=M(z);
    double dm=dcr0*Om*pow((1.+z),3.);
...
{
  int i,j,k;
    double mdot_tot, mdot_cold, mtot, mcold, mwarm, mdot_acc, *dx1, *x1, *dx2, *x2, *dx3, *x3, dvol;
    double sendarray[6], recvarray[6];
    mdot_tot=0;
    mdot_cold=0;
    mtot=0;
    mcold=0;
    ...
    int rankcoord=5*grid->nproc[IDIR]/grid->np_tot_glob[IDIR];
    #ifdef PARALLEL
     if(grid->rank_coord[0] == rankcoord){ //calculate Mdot accreting through the inner radius
     #endif 
    KDOM_LOOP(k){
    JDOM_LOOP(j)
    {
        if(x2_glob[j]>0.2 && x2_glob[j]<2.94)
        {
            i=5-rankcoord*grid->np_int[IDIR];
            double mu  = MeanMolecularWeight(d->Vc);
            double T=d->Vc[PRS][k][j][i]/d->Vc[RHO][k][j][i]*KELVIN*mu;
            mdot_tot += x1[i]*x1[i]*sin(x2[j])*dx2[j]*dx3[k]*d->Vc[RHO][k][j][i]*d->Vc[VX1][k][j][i];
            if(T<1.e5) mdot_cold += x1[i]*x1[i]*sin(x2[j])*dx2[j]*dx3[k]*d->Vc[RHO][k][j][i]*d->Vc[VX1][k][j][i];
            if(T<1.e5) mdot_acc += x1[i]*x1[i]*sin(x2[j])*dx2[j]*dx3[k]*d->Vc[RHO][k][j][i]*d->Vc[VX1][k][j][i];  
        }
    }}
    #ifdef PARALLEL 
    }
    ...
    DOM_LOOP(k,j,i)
    {
        if(x2_glob[j]>0.2 && x2_glob[j]<2.94)
        {
            ...
            if(x1[i]*UNIT_LENGTH<5.e3*CONST_pc) mtot+=d->Vc[RHO][k][j][i]*dvol;
            if(T<1.e5 && x1[i]*UNIT_LENGTH<5.e3*CONST_pc) mcold+=d->Vc[RHO][k][j][i]*dvol;
            if(T>1.e5 && T<=1.e8 && x1[i]*UNIT_LENGTH<5.e3*CONST_pc) mwarm+=d->Vc[RHO][k][j][i]*dvol;
            //if(T<1.e5 && x1[i]*UNIT_LENGTH<5.e3*CONST_pc) mdot_acc+=d->Vc[RHO][k][j][i]*dvol;	
        }
    }
     #ifdef PARALLEL
     ...
     sendarray[3] = mcold;
     sendarray[4] = mwarm;
     sendarray[5] = mdot_acc;
     MPI_Allreduce (sendarray, recvarray, 6, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
     mdot_tot = recvarray[0];
     mdot_cold = recvarray[1];
     mtot = recvarray[2];
     mcold = recvarray[3];
     mwarm = recvarray[4];
     mdot_acc = recvarray[5];
     #endif

    double tau_dep=200.0*3.154e7*1.e6/(UNIT_LENGTH/UNIT_VELOCITY);
    double eps_r=0.1;
    ...
    mdot_acc = MIN(mdot_acc, Mdot_edd);

    M_BH = M_BH + mdot_acc*eps_r*(UNIT_DENSITY*UNIT_LENGTH*UNIT_LENGTH*UNIT_VELOCITY)*g_dt*(UNIT_LENGTH/UNIT_VELOCITY);
    ...
    fp = fopen(fname,"w"); /*
    from beginning */
    fprintf (fp,"# %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s\n", "t", "dt", "mdot_acc", "mdot_edd", "mdot_tot", "mdot_cold", "m_tot", "m_cold", "m_warm", "mdot_jet", "TEdot_jet", "M_BH");
    }else{
    ...
    fprintf (fp, "%12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e %12.6e \n",g_time, g_dt, mdot_acc, Mdot_edd, mdot_tot,mdot_cold, mtot, mcold, mwarm, mdot_jet, TEdot_jet, M_BH);
    }
    fclose(fp);
    }
...
{ 
  ...
  {  
    ...
    {
	#ifdef PARALLEL
            if (grid->rank_coord[0] == grid->nprocs[IDIR]-1){
        #endif 
        double z=solver(g_time*UNIT_LENGTH/UNIT_VELOCITY);
        BOX_LOOP(box,k,j,i)
        {
            // d->Vc[VX1][k][j][i]=(H0*Ez(z))*(x1[i]*UNIT_LENGTH)/UNIT_VELOCITY;
            d->Vc[RHO][k][j][i] = d->Vc[RHO][k][j][i-1];
            d->Vc[PRS][k][j][i] = d->Vc[PRS][k][j][i-1];
            // d->Vc[PRS][k][j][i]=2.e4*d->Vc[RHO][k][j][i]/KELVIN/mu;
